...
@section('content')
    <div id="main">
        ...
        @endif
        <h3 style="padding-top: 50px;">Ürünü Güncelle</h3>
        <div class="container" style="padding-top: 50px;">
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-floating mb-3">
                        <select class="form-select" name="category" id="urunKategorisi" aria-label="Ürün Kategorisi">
                            @foreach($categories as $category)
                                <option @if($product->getCategory->id == $category->id) selected @endif value="{{$category->id}}">{{$category->name}}</option>
                            @endforeach
                        </select>
                        <label for="urunKategorisi">Ürün Kategorisi</label>
                    </div>
                    <div class="form-floating mb-3">
                        <input class="form-control" id="marka" name="brand" type="text" placeholder="Marka" value="{{$product->brand}}" data-sb-validations="required" />
                        <label for="marka">Marka</label>
                        <div class="invalid-feedback" data-sb-feedback="marka:required">Marka is required.</div>
                    </div>
                    <div class="form-floating mb-3">
                        <input class="form-control" id="model" name="model" type="text" placeholder="Model" value="{{$product->model}}"  data-sb-validations="required" />
                        <label for="model">Model</label>
                        <div class="invalid-feedback" data-sb-feedback="model:required">Model is required.</div>
                    </div>
                    <div class="form-floating mb-3">
                        <textarea class="form-control" id="ozellikler" name="features" type="text" placeholder="Özellikler" style="height: 18rem;" data-sb-validations="required">{{$product->features}}</textarea>
                    ...
                    <div class="form-floating mb-3">
                        <input class="form-control" id="fiyat" name="price" type="text" value="{{$product->price}}" placeholder="Fiyat" data-sb-validations="required" />
